...
% The lambda's can also be packaged as a cell array in which case 
% Skellam(x,params) implies parameters is a cell array with params{1} as
% lambda1's and params{2} as lambda2's
%
%
...
if isscalar(x)
    % error checking
    if floor(x)-x ~= 0
        error('SkellamPMF:NonInteger','x must be integer');
    end
    if lambda1==0 
